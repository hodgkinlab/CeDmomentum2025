"""
Modify the model to freely estimate the initial cell numbers.
Last edit: 5-July-2023
"""
import numpy as np
cimport numpy as np
np.get_include()
...
class Cyton2Model:
	def __init__(self, ht, max_div, dt, nreps, logn=True):
		self.t0 = <DTYPE_t>(0.0)
	...
	def evaluate(self,
			DTYPE_t n0,                    # initial cell numbers
		...
		cdef DTYPE_t x, y
		nUNS = n0 * (1. - p) * sfUns
		nDIV[0,:] = n0 * p * sfDie * sfDiv * sfDD
		nDES[0,:] = n0 * p * sfDie * np.cumsum([x * y for x, y in zip(pdfDD, sfDiv)]) * self.dt
