...
All figures are located in $\texttt{out}$ folder and organised by the cell type.

- $\texttt{analysis}$ folder contains figures generated from the Jupyter notebook.
- $\texttt{TCM}$, $\texttt{Block}$ and $\texttt{Block+IL-2}$ folders contain Cyton2 model fit results for each CD4+ and CD8+ samples.
